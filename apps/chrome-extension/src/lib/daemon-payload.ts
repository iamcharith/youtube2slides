...
export function buildSummarizeRequestBody({
  ...
  const slidesSettings = slidesEnabled
    ...
    : {};
  if (inputMode === "video") {
    const hasPageTranscript = isYouTubeWatchUrl(extracted.url) && extracted.text.trim().length > 0;
    const videoOverrides = hasPageTranscript
      ? { mode: "page" }
      : isYouTubeWatchUrl(extracted.url)
        ? { mode: "url" }
