...
function normalizePromptOverride(value: unknown): string {
  ...
  const trimmed = value.trim();
  if (!trimmed) return defaultSettings.promptOverride;
  return value;
}
...
export const defaultSettings: Settings = {
  ...
  language: "auto",
  promptOverride:
    "Turn this source into a slide-ready outline for a presentation. Use concise slide titles, speaker notes, and key takeaways. Preserve important timestamps when available.",
  maxChars: 120_000,
  requestMode: "",
  firecrawlMode: "",
  markdownMode: "",
