...
function extractYouTubeTranscriptText(
  maxChars: number,
  ...
  const lines = segments
    .map((segment) => (segment as HTMLElement).innerText.replace(/\s+/g, " ").trim())
    .filter(Boolean);
  if (lines.length === 0) return null;
  const text = lines.join("\n");
  const clamped = clampText(text, maxChars);
  return clamped.text.length > 0 ? clamped : null;
}
...
function extract(maxChars: number): ExtractResponse {
  try {
    ...
    const youtubeTranscript = extractYouTubeTranscriptText(maxChars);
    if (youtubeTranscript) {
      return {
        ...
        truncated: youtubeTranscript.truncated,
