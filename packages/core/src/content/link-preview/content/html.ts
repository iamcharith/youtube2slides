...
export async function buildResultFromHtmlDocument({
  ...
  if (
    ...
    transcriptResolution.text === null &&
    transcriptResolution.source === "unavailable" &&
    !youtubeDescription
  ) {
    const notes = transcriptResolution.diagnostics?.notes
      ? ` (${transcriptResolution.diagnostics.notes})`
      : "";
    throw new Error(`YouTube transcript unavailable${notes}`);
  }
