...
export function createUrlExtractionSession({
  ...
  const client = createLinkPreviewClient({
    ...
    ytDlpPath: resolveUrlFlowYtDlpPath({
      ...
      allowGuardedExternalDownloader: urlUtils.isYouTubeVideoUrl(targetUrl),
    }),
    transcription: {
      env: io.envForRun,
    ...
    try {
      ...
      const preferUrlMode =
        typeof urlUtils.shouldPreferUrlMode === "function"
          ...
          : false;
      const isYouTubeVideo = urlUtils.isYouTubeVideoUrl(targetUrl);
      const isTwitter = urlUtils.isTwitterStatusUrl?.(targetUrl) ?? false;
      const isPodcast = urlUtils.isPodcastHost?.(targetUrl) ?? false;
      if (!preferUrlMode || isYouTubeVideo || isTwitter || isPodcast) throw err;
      writeVerbose(
        io.stderr,
        flags.verbose,
